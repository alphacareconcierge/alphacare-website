  ...
  return (
    <header className="sticky top-0 z-50 border-b border-navy/10 bg-ivory">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-8 px-5 py-4 sm:px-8">
        ...
        <nav className="hidden items-center gap-10 lg:flex" aria-label="Primary navigation">
          {navItems.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="type-nav uppercase tracking-[0.16em] text-navy/78 transition hover:text-navy focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-gold"
            >
              {item.label}
            </Link>
          ))}
        ...
        <Link
          href="/contact"
          className="type-cta hidden border border-gold px-4 py-2 text-navy transition hover:bg-gold"
        >
          Start
        </Link>
      </div>
      <nav
        className="flex justify-center gap-6 border-t border-navy/10 px-5 py-3 lg:hidden"
